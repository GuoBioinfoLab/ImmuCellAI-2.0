...
out_dir <- file.path(fig4_dir, "TCGA_ImmunotherapyResponsive_ImmuCellAI2_marker5000")
...
tumor_sample_type_codes <- c("01", "02", "03", "05", "06")
...
log_file <- file.path(out_dir, "TCGA_ImmunotherapyResponsive_ImmuCellAI2_marker5000_run.log")
...
project_case_map <- unique(clinical[, c("case_submitter_id", "project_id")])
project_case_map <- project_case_map[project_case_map$project_id %in% immunotherapy_projects, , drop = FALSE]
project_case_map <- project_case_map[nzchar(project_case_map$case_submitter_id), , drop = FALSE]
if (nrow(project_case_map) == 0L) stop("No selected immunotherapy-responsive projects found in clinical.tsv")

log_message("Reading TCGA TPM header...")
all_samples <- scan(tcga_tpm_file, what = "", nlines = 1L, sep = "\t", quiet = TRUE)
sample_cases <- substr(all_samples, 1L, 12L)
sample_type_codes <- substr(all_samples, 14L, 15L)
case_to_project <- setNames(project_case_map$project_id, project_case_map$case_submitter_id)

keep_samples <- sample_cases %in% names(case_to_project) & sample_type_codes %in% tumor_sample_type_codes
selected_samples <- all_samples[keep_samples]
if (length(selected_samples) == 0L) stop("No expression samples matched selected projects and tumor sample type codes")

sample_info <- data.frame(
  Sample = selected_samples,
  CaseSubmitterID = substr(selected_samples, 1L, 12L),
  ProjectID = unname(case_to_project[substr(selected_samples, 1L, 12L)]),
  SampleTypeCode = substr(selected_samples, 14L, 15L),
  SampleType = sample_type_label(substr(selected_samples, 14L, 15L)),
  stringsAsFactors = FALSE
)
sample_info <- unique(sample_info)
sample_info <- sample_info[order(sample_info$ProjectID, sample_info$Sample), ]
selected_samples <- sample_info$Sample

write.table(sample_info, file.path(out_dir, "TCGA_ImmunotherapyResponsive_tumor_sample_info.txt"),
            sep = "\t", quote = FALSE, row.names = FALSE)
project_counts <- as.data.frame.matrix(table(sample_info$ProjectID, sample_info$SampleTypeCode))
project_counts <- data.frame(ProjectID = rownames(project_counts), project_counts, check.names = FALSE, row.names = NULL)
write.table(project_counts, file.path(out_dir, "TCGA_ImmunotherapyResponsive_project_sample_counts.txt"),
            sep = "\t", quote = FALSE, row.names = FALSE)

log_message("Selected projects: ", paste(immunotherapy_projects, collapse = ", "))
log_message("Selected unique cases: ", length(unique(sample_info$CaseSubmitterID)))
log_message("Selected tumor expression samples: ", length(selected_samples))
...
select_cols <- c(1L, match(selected_samples, all_samples) + 1L)
selected_expr_file <- file.path(out_dir, "TCGA_ImmunotherapyResponsive_tumor_TPM_symbol.txt")
if (!file.exists(selected_expr_file)) {
  log_message("Extracting selected tumor expression matrix from full TCGA TPM file...")
  selected_dt <- fread(
    tcga_tpm_file,
    sep = "\t",
    header = FALSE,
    skip = 1L,
    select = select_cols,
    col.names = c("Gene", selected_samples),
    check.names = FALSE,
    data.table = TRUE,
    showProgress = TRUE
  )
  fwrite(selected_dt, selected_expr_file, sep = "\t", quote = FALSE)
  rm(selected_dt)
  gc()
} else {
  log_message("Reusing selected expression matrix: ", selected_expr_file)
}

log_message("Reading reference and selected matrix...")
reference <- read_matrix_gene_first(reference_file)
bulk <- read_matrix_gene_first(selected_expr_file)
...
if (length(common) < 100L) stop("Too few marker genes overlap selected bulk and reference: ", length(common))
...
filtered_file <- file.path(out_dir, "TCGA_ImmunotherapyResponsive_tumor_TPM_symbol_marker5000_filtered.txt")
...
for (i in seq_along(chunks)) {
  ...
  write.table(status, file.path(out_dir, "TCGA_ImmunotherapyResponsive_ImmuCellAI2_chunk_status.txt"),
              sep = "\t", quote = FALSE, row.names = FALSE)
...
write.table(
  ...
  file.path(out_dir, "TCGA_ImmunotherapyResponsive_ImmuCellAI2_marker5000_state_fraction_sample_by_celltype.txt"),
  sep = "\t",
  quote = FALSE,
  col.names = NA
)
write.table(
  ...
  file.path(out_dir, "TCGA_ImmunotherapyResponsive_ImmuCellAI2_marker5000_state_fraction_celltype_by_sample.txt"),
  sep = "\t",
  quote = FALSE,
  col.names = NA
)
write.table(
  ...
  file.path(out_dir, "TCGA_ImmunotherapyResponsive_ImmuCellAI2_marker5000_major_fraction_sample_by_lineage.txt"),
  sep = "\t",
  quote = FALSE,
  col.names = NA
)
write.table(hierarchy, file.path(out_dir, "TCGA_ImmunotherapyResponsive_ImmuCellAI2_marker5000_hierarchy.txt"),
            sep = "\t", quote = FALSE, row.names = FALSE)

run_info <- data.frame(
  Package = "ImmuCellAI2.0",
  Version = as.character(packageVersion("ImmuCellAI2.0")),
  ProjectSet = "ImmunotherapyResponsive",
  Projects = paste(immunotherapy_projects, collapse = ","),
  SelectedCases = length(unique(sample_info$CaseSubmitterID)),
  Samples = ncol(bulk),
  GenesUsed = nrow(bulk),
  ...
  NIter = 50,
  VBTolerance = 1e-6,
  TumorSampleTypeCodes = paste(tumor_sample_type_codes, collapse = ","),
  stringsAsFactors = FALSE
)
write.table(run_info, file.path(out_dir, "TCGA_ImmunotherapyResponsive_ImmuCellAI2_marker5000_run_info.txt"),
            sep = "\t", quote = FALSE, row.names = FALSE)
saveRDS(
  list(
    ...
    project.counts = project_counts,
    hierarchy = hierarchy,
    run.info = run_info
  ),
  file.path(out_dir, "TCGA_ImmunotherapyResponsive_ImmuCellAI2_marker5000_results.rds"),
  compress = FALSE
)
